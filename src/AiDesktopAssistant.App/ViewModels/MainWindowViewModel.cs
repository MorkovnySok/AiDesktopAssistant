...
public sealed class MainWindowViewModel : ViewModelBase
{
    ...
    private readonly ISelectedTextProvider _selectedTextProvider;
        ...
        ILogger<MainWindowViewModel> logger)
    {
        ...
        _selectedTextProvider = selectedTextProvider;
        _logger = logger;
    ...
    public async Task<string?> ActivateFromHotkeyAsync(CancellationToken cancellationToken = default)
    {
        var selectedText = await _selectedTextProvider.GetSelectedTextAsync(cancellationToken);
        if (!string.IsNullOrWhiteSpace(selectedText))
        {
            UserInput = BuildSelectedTextPrompt(selectedText);
        }
    ...
    private static string BuildSelectedTextPrompt(string selectedText) =>
        $"Use this selected text from the screen as context:\n\n```text\n{selectedText}\n```\n\nMy question: ";
}
